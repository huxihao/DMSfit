...
glmDispersion <- function(model.glm) {
	theta = sum(residuals(model.glm, type="pearson")^2)/df.residual(model.glm)
	return(max(c(1,theta)))
}
...
train.PoissonLinear <- function(data, thrd=1e-4, max_iter=80) {
	...
	for (k in 1:max_iter) {
		## Step 2
		model.glm <- glm(count ~ . - index, data = data, family = quasipoisson(link = "log"), offset = log_expr)
...
train.MixturePoissonLinear_seed <- function(data, thrd=1e-4, max_iter=80, seed=2012) {
	...
	for (k in 1:max_iter) {
		## Step 3
		model.glm0 <- glm(count ~ . - index, data = data, family = quasipoisson(link = "log"), offset = log_expr0, weights = rho0)
		model.glm1 <- glm(count ~ . - index, data = data, family = quasipoisson(link = "log"), offset = log_expr1, weights = rho1)
		...
		dispersion <- sqrt((glmDispersion(model.glm0) + glmDispersion(model.glm1)))
		log_pred_prob0 <- dnorm(data$count, exp(log_pref0), exp(log_pref0*0.5)*dispersion, log=TRUE)
		log_pred_prob1 <- dnorm(data$count, exp(log_pref1), exp(log_pref1*0.5)*dispersion, log=TRUE)
	...
	null_dev <- getDev(getNullCount(data), data$count)
	r_squared <- 1 - now_dev/null_dev
	cat('Dispersions:', glmDispersion(model.glm0), glmDispersion(model.glm1), '\n')
	cat("R-squared =", r_squared,"\n")
...
{
	## use synthetic data 
#	write("Synthetic data ProbRNA", file="log.txt", append=TRUE)
#	write("Ratio\tLeftSpan\tRightSpan\tPL\tMP\tMPL", file="log.txt", append=TRUE)
#	for(ratio in seq(0, 1, 0.1))
#		fit_pars("../work/ProbRNA.csv", ratio)

	## use real data
	write("Real data ProbRNA", file="log.txt", append=FALSE)
	fit_pars("../work/ProbRNA.csv")
}
