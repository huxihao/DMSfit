...
#			count[run_i] <- rep(0, length(oriData$index[i])) ## skip the count data, processed other ways
	...
	return(data)
	#return(data[,-(llen+3)]) ## remove position 0
}
...
glmPred_old <- function(model.glm, newdata) {
	#m <- length(characters)
...
train.MixturePoissonLinearCombine_seed <- function(data, same_dir=TRUE, thrd=1e-4, max_iter=80, seed=2012) {
	...
	for (k in 1 : max_iter) {
		## Step 3
		model.glm0v <- glm(count ~ .-index, data = data$v1, family = quasi(link = "log", variance = "mu^2"), offset = log_expr0v, weights = rho0)
		model.glm1v <- glm(count ~ .-index, data = data$v1, family = quasi(link = "log", variance = "mu^2"), offset = log_expr1v, weights = rho1)
		model.glm0s <- glm(count ~ .-index, data = data$s1, family = quasi(link = "log", variance = "mu^2"), offset = log_expr0s, weights = rho0)
		model.glm1s <- glm(count ~ .-index, data = data$s1, family = quasi(link = "log", variance = "mu^2"), offset = log_expr1s, weights = rho1)
		...
		dispersion_v <- sqrt((glmDispersion(model.glm0v) + glmDispersion(model.glm1v)))
		log_pred_prob0v <- dnorm(data$v1$count, exp(log_pref0v), exp(log_pref0v*0.5)*dispersion_v, log=TRUE)
		log_pred_prob1v <- dnorm(data$v1$count, exp(log_pref1v), exp(log_pref1v*0.5)*dispersion_v, log=TRUE)

		dispersion_s <- sqrt((glmDispersion(model.glm0s) + glmDispersion(model.glm1s)))
		log_pred_prob0s <- dnorm(data$s1$count, exp(log_pref0s), exp(log_pref0s*0.5)*dispersion_s, log=TRUE)
		log_pred_prob1s <- dnorm(data$s1$count, exp(log_pref1s), exp(log_pref1s*0.5)*dispersion_s, log=TRUE)
